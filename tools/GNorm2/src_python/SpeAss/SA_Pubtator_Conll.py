...
def corpus_noNest(token_input):
    ...
    for doc in documents:
        ...
        for ele in entity_list:
            if ele[4]=='Gene' and re.fullmatch(r'\d+\(Species:\d+\)', ele[5]):
                temp_gene={}
                gene_ids=ele[5].split(',')
                for gene_id in gene_ids:
                    temp_id=gene_id[gene_id.find('Species:'):-1]
                    spe_id = temp_id[len('Species:'):].strip(')')  # 去掉前綴並移除可能的括號
...
if __name__=='__main__':
    ...
    parser.add_argument('--inputfile', '-i', help="inputfile in PubTator format",default='corpus/ori_sp_pubtator/merge.TrainingList.6.Sp.PubTator')
    parser.add_argument('--outputfile', '-o', help="outputfile in BIO-conll format",default='corpus/ori_sp_pubtator/merge.TrainingList.6.Sp.conll')
