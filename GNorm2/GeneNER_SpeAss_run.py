...
#nlp_token = stanza.Pipeline(lang='en', processors={'tokenize': 'spacy'},package='None') #package='craft'
...
#nlp_token = stanza.Pipeline(model_dir='gnorm_trained_models/stanza', lang='en', processors={'tokenize': 'spacy'})

def NER_BioC(infolder, infile, outpath, nn_model):
    """
    If the input is BioC XML, read it, run NER, and write output to BioC XML.
    """
    with open(os.path.join(infolder, infile), 'r', encoding='utf-8') as fin:
        with open(os.path.join(outpath, infile), 'w', encoding='utf8') as fout:
            ...
            for document in collection.documents:
                print(f"Processing: {round(pmid_n * 100 / total_n)}%", end="\r")
                pmid_n += 1

                mention_num_new = 0
                for passage in document.passages:
                    # Skip ref passages or empty texts
                    if passage.text and not passage.text.isspace() and passage.infons.get('type') != 'ref':
                        ...
                        for ele in tag_result:
                            ...
                            loc = bioc.BioCLocation(
                                offset=str(passage_offset + start),
                    ...
                    # Re-assign annotation IDs to avoid duplicates
...
def NER_PubTator(infolder, infile, outpath, nn_model):
    """
    If the input is PubTator format, run NER and save back in PubTator.
    """
    with open(os.path.join(infolder, infile), 'r', encoding='utf-8') as fin:
        with open(os.path.join(outpath, infile), 'w', encoding='utf-8') as fout:
            ...
            for doc in all_text:
                ...
                seg = lines[0].split('|t|')
                pmid = seg[0]
                title = seg[1] if len(seg) > 1 else ""

                abstract = ""
                if len(lines) > 1:
                    seg_abs = lines[1].split('|a|')
                    if len(seg_abs) > 1:
                        abstract = seg_abs[1]
                ...
                fout.write(doc + '\n')
                for ele in tag_result:
                    ent_start, ent_last, ent_type = ele
                    ent_mention = intext[int(ent_start):int(ent_last)]
                    fout.write(f"{pmid}\t{ent_start}\t{ent_last}\t{ent_mention}\t{ent_type}\n")
                fout.write('\n')
                
def geneNER(infolder, outpath, modelfile):
    """
    1) Load the trained Gene NER model
    2) Tag input (BioC or PubTator)
    """
    
    # Decide which checkpoint to use
    print('loading NER models........')
    if 'bioformer' in modelfile.lower():
        vocabfiles = {
            'labelfile': './vocab/GeneNER_label.vocab',
    ...
    if fin_pubtator2.getvalue()!='':
        ...
        for doc in all_in:
            ...
            for _spe_ele in doc_species:
                ...
                elif _spe_ele[2:] in prefix_speid_allset:
                    for ele in prefix_dict[_spe_ele[2:]]:
                        _prefix_str2id_dict[ele]=_spe_ele[2:]
        ...
        for passage in document.passages:
            for temp_annotation in passage.annotations:
                if 'Identifier' not in temp_annotation.infons.keys():
                    if doc_pmid in final_sa_results:
        ...
        for i in range(2,len(lines)):
            ...
            if len(segs)>=6: #species
                ...
                if species_ID.find('*')>=0:
                    ...
                    elif species_ID[2:] in prefix_speid_allset:
                        for ele in prefix_dict[species_ID[2:]]:
                            _prefix_str2id_dict[ele]=species_ID[2:]
            else: #gene
...
def speciesAss(infolder,outpath, modelfile):
    ...
    if model_type == 'bioformer':
        vocabfiles = {
            'labelfile': './vocab/SpeAss_IO_label.vocab',
            'checkpoint_path': './gnorm_trained_models/bioformer-cased-v1.0/',
            'lowercase': False,
            'from_pt': False
        }
    ...
    dict_filename = {
        'prefix': './Dictionary/SPPrefix.txt',
        'virus': './Dictionary/SP_Virus2HumanList.txt'
    }

    with open(dict_filename['virus'], 'r', encoding='utf-8') as fin:
        virus_set = set(fin.read().strip().split('\n'))

    prefix_dict = {}
    with open(dict_filename['prefix'], 'r', encoding='utf-8') as fin:
        for line in fin:
            seg = line.strip().split('\t')
            if seg[0] not in prefix_dict:
...
if __name__=='__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Run Gene NER and/or Species Assignment. Example usage:\n'
                    'python GeneNER_SpeAss_run.py -i input_dir -n models/NER.h5 -s models/SA.h5 -r ner_out -a sa_out'
    )
    parser.add_argument('--infolder', '-i', default='./example/input/', help="input folder")
    parser.add_argument('--NERmodel', '-n', default='', help="trained deep learning NER model file")
    parser.add_argument('--SAmodel', '-s', default='', help="trained deep learning species assignment model file")
    parser.add_argument('--NERoutpath', '-r', default='./example/ner_output/', help="output folder for NER results")
    parser.add_argument('--SAoutpath', '-a', default='./example/sa_output/', help="output folder for SA results")
    parser.add_argument('--NUM_THREADS', '-t', default='3', help="Number of threads")
    args = parser.parse_args()

    # (Optional) Force Eager Mode to avoid certain TF graph issues
    tf.config.run_functions_eagerly(True)

    # Threading
    if not args.NUM_THREADS.isdigit():
        args.NUM_THREADS = '3'
    ...
    if args.NERoutpath and not args.NERoutpath.endswith('/'):
        args.NERoutpath += '/'
    if args.NERoutpath and not os.path.exists(args.NERoutpath):
        os.makedirs(args.NERoutpath)

    if args.SAoutpath and not args.SAoutpath.endswith('/'):
        args.SAoutpath += '/'
    if args.SAoutpath and not os.path.exists(args.SAoutpath):
        os.makedirs(args.SAoutpath)

    #################################
    # 5) Main Branching Logic
    #################################
    if args.NERmodel and args.SAmodel:
        print('==============\n| GeneNER and SpeAss |\n==============')
        # 1) Run Gene NER
        geneNER(args.infolder, args.NERoutpath, args.NERmodel)
        # 2) Run Species Assignment on the NER output
        speciesAss(args.NERoutpath, args.SAoutpath, args.SAmodel)

    elif args.NERmodel and not args.SAmodel:
        print('==============\n| GeneNER |\n==============')
        geneNER(args.infolder, args.NERoutpath, args.NERmodel)

    elif not args.NERmodel and args.SAmodel:
        print('==============\n| SpeAss |\n==============')
        speciesAss(args.infolder, args.SAoutpath, args.SAmodel)
    else:
        print('Please provide at least one model via --NERmodel or --SAmodel.')
